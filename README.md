...
In the original BRIE model, each user is associated with a learned embedding indexed by a unique identifier. While effective, this approach presents two fundamental limitations:

- The number of parameters grows linearly with the number of users.
- The model must be retrained whenever new users are incorporated.
- It cannot naturally handle user cold-start scenarios.

BRIE-DeepSets removes these limitations by representing users as permutation-invariant sets of visual embeddings. The user representation is computed dynamically from image content, eliminating the need for stored user-specific parameters.

As a result:

- The number of parameters becomes **constant across datasets**, independent of the number of users.
- The model naturally supports **new unseen users** without architectural changes.
- The system becomes scalable to real-world deployment scenarios.

The proposed approach preserves the Bayesian Pairwise Ranking (BPR) objective and the explainability-oriented design of BRIE, while addressing scalability and cold-start limitations.
...
- Elimination of user-specific parameters
- Constant model size regardless of dataset scale
- Natural support for user cold-start
...
- Explicit analysis of scalability vs ranking performance trade-off
...
The primary objective of BRIE-DeepSets is **scalability**, not marginal ranking improvements.

Special emphasis is placed on the **number of parameters**, as the main contribution is removing the dependency between model size and number of users.
...
| ELVis | 8,734,913 | 0.530 | 0.293 | 0.629 |
| MF-ELVis | 8,726,592 | 0.531 | 0.267 | 0.597 |
...
- The number of parameters is constant (**209,472**) across all datasets.
- In the original BRIE, parameters grow linearly with user count (millions in large cities).
- Cold-start users are naturally supported, since no user-specific embeddings are stored.
- Ranking performance remains consistently competitive.
- The small performance gap is outweighed by the scalability gain.

The reduction from millions of parameters to a constant-size architecture is the key contribution of this work.
