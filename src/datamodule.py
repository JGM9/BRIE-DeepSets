...
class TripadvisorImageAuthorshipBPRDataset(TripadvisorImageAuthorshipBCEDataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.set_type == "train":
            ...
            # Obtenemos los embeddings de las imágenes del usuario
            image_ids = self.user_to_image_ids[user_id][:self.max_user_images]  # lista recortada de image_ids (máx 20)
            image_tensors = [self.datamodule.image_embeddings[i] for i in image_ids]  # lista de tensores (1536,)

            # Padding hasta max_user_images
            pad_len = self.max_user_images - len(image_tensors)  # número de ceros a añadir
            if pad_len > 0:
                pad_tensor = torch.zeros_like(image_tensors[0])  # tensor (1536,) de ceros
                image_tensors += [pad_tensor] * pad_len           # añadimos el padding

            user_images = torch.stack(image_tensors)  # tensor final (max_user_images, 1536)
        ...
        # inside the LightningModule     
            ...
            image_tensors = [self.datamodule.image_embeddings[i] for i in image_ids]
